...
Three metrics should be continuously monitored once the agent is live. Each covers a different layer of the system: business outcome, infrastructure health, and AI-specific quality.

1. Escalation Rate: Percentage of total conversations where the Chat Service triggers the handoff path to a human agent. This is the primary business metric, a rising escalation rate means the agent is failing to resolve queries, whether from knowledge base gaps, poor retrieval, or low-confidence LLM responses. Measured by counting escalation events divided by total conversations

2. End-to-End Response Latency: Time from request entry at the Chat Service to response exit, reported at `p50`, `p95`, and `p99` percentiles. Percentiles matter more than averages because telco traffic is spiky (billing cycles, outages) and `p95`/`p99` reflects actual user experience during peak load. For chat, anything above 3–5 seconds feels broken. Measured by instrumenting the Chat Service with sub-spans for each downstream call

3. Retrieval Relevance: Semantic match quality between the user query and the top-k chunks returned from the Embeddings Store. This is a leading indicator — retrieval degrades before customer-visible answer quality does. Poorly chunked documents or embedding drift will cause the LLM to confidently generate wrong answers from irrelevant context. Measured by logging retrieved chunks alongside each query, then scoring via: a. cosine similarity b. llm as judge

### Toolings

While I don't have experience in selecting observability toolings, in the architecture diagram a custom tooling/logging is proposed by using an Object Storage. But on another occasion I would research the custom tooling with other toolings with these criterias:

1. Open source: Preference for self-hostable solutions over vendor-locked SaaS. A telco handles customer conversation data that may be subject to data residency and regulatory compliance; self-hosting ensures sensitive logs and traces never leave the company's infrastructure

2. Popularity / Quality: The tooling should have an active community and established track record in LLM observability specifically, not just general APM. Purpose-built tools that natively understand concepts like traces, generations, retrieval steps, and prompt versioning will require less custom instrumentation than adapting a generic monitoring platform

3. Price: Total cost of ownership at production scale, accounting for trace volume generated by high-throughput telco traffic. The tooling should offer predictable pricing (or be free if self-hosted) that doesn't scale linearly with conversation volume during traffic spikes like billing cycles or outages
